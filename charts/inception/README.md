...
| auth.oauth2.roleMapping.claim | string | `""` | OAuth2 claim holding the mapping informations (ex : groups, or roles). |
| auth.oauth2.roleMapping.adminRole | string | `""` | OAuth2 claim value to map to INCEpTION's `Admin` role. |
| auth.oauth2.roleMapping.userRole | string | `""` | OAuth2 claim value to map to INCEpTION's `User` role. |
| auth.oauth2.roleMapping.projectCreatorRole | string | `""` | OAuth2 claim value to map to INCEpTION's `Project Creator` role. |
| auth.oauth2.roleMapping.remoteRole | string | `""` | OAuth2 claim value to map to INCEpTION's `Remote` role. |
